...
- **F5 redirige a `/login` y se queda en spinner infinito (admin confirmado)**: tras F5 sobre `/admin`, el bloque "Terminal null" de `AdminShell` se disparaba prematuramente (cuando `currentProfileProvider` retornaba `AsyncData(null)` por un tick transitorio antes de que `currentUserProvider` propagara el User) y enviaba al admin a `/login` antes de que el profile re-evaluara y cargara correctamente. Adicionalmente, el redirect del router desde `/login → /admin` no re-disparaba de forma confiable cuando el profile cargaba (el `AuthRefreshNotifier` notificaba pero GoRouter no re-evaluaba el redirect en la ubicación actual). Fix en dos partes: (1) el bloque "Terminal null" de `AdminShell` ahora solo redirige a `/login` si `currentUserProvider != null`; (2) la rama "spinner while logged in" de `LoginScreen` ahora observa el profile y navega activamente con `context.go(isAdmin ? '/admin' : '/dashboard')` cuando el profile resuelve, bypaseando el redirect del router.
