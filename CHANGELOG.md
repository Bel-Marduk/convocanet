...
### Fixed
- **Click "Ver" / "Ver sitio" en admin no navega a `/convocatoria/:id` ni `/`**: el `AdminShell` quedaba "keep alive" en memoria porque su state está suscrito a `routerDelegate.addListener()`. Al navegar a un `GoRoute` top-level fuera de `/admin/*`, el shell no se desmontaba, el listener seguía disparando, `_cachedLocation` se actualizaba, y `computeSelectedIndex` caía al default (0 = admin dashboard), secuestrando la pantalla. El `ConvicatoriaDetailScreen` y `LandingScreen` nunca se construían. Fix: guard en `AdminShell.build` que retorna `const SizedBox.shrink()` si la URL no empieza con `/admin`, dejando que el `GoRoute` top-level tome la pantalla.
